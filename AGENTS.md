...
- 2026-09-05 — **Native Windows smoke**: the locally built 3.27.1 executable was launched by its full process path. Confirmed navigation, a General field label remaining visible after entering sample text, full wizard step names with invalid-config progression disabled, and the report's disabled snapshot actions plus prerequisite hint. Closed both test-launched windows afterward. No module installation, sign-in, or tenant operations were performed. The screenshot harness now matches the `-de` suffix exactly, so `validation-details` no longer accidentally selects German.

- 2026-09-05 — **Import schedule validation**: configuration JSON must carry all seven weekdays, including disabled days. This prevents imported partial schedules from silently removing editable days or changing opening hours; rejection happens before import preview/application. The regression removes a weekday from serialized JSON and verifies the actionable validation error. Production ScriptBuilders remain untouched.

- 2026-09-05 — **Beginner telephony guidance (3.27.2)**: Welcome now explains auto attendants, call queues/agents, resource accounts and tenants with a reception example. Readiness explains both connections, preparation needs, and the boundary between connected services and a validated phone system; Configuration explains that editing the form does not provision resources. English/German expanded and collapsed help plus connection-ready states are in ScreenshotGenerator. Native Windows help expansion and Welcome-to-Readiness navigation were checked without module installation, sign-in, or tenant mutation.
- 2026-09-07 — **Issue #71 full live snapshots (3.28.0)**: topology schema v2 combines a newly queried Dashboard topology with structured data for every Tenant Report source (tenant, detailed AA/CQ flows, resource-account associations, schedules, phone-number assignments, and voice users). Comparisons validate the file before tenant access and fail closed on query errors, warnings, missing markers, or partial Dashboard sections; schema-v1 snapshots remain readable for their original fields. Nested drift is reported as scalar property changes. Detail rows that frozen markers identify only by display name stay flat: unique names resolve to the real parent identity, while duplicates remain unresolved and use deterministic full-row multiset identities, preventing invented links or misattributed changes. Resource-account associations always use account/type/configuration/occurrence identity so same-type rows cannot collide. Existing Documentation/Dashboard ScriptBuilder output and authentication remain byte-identical; all snapshot queries are read-only. The full English/German headless matrix was visually inspected. Windows automation exposed only the older installed build, not the local 3.28.0 build, so native 3.28.0 and tenant-connected validation remain separate pilot gates.
- 2026-09-08 — **Issue #78 tenant health checks (3.29.0)**: the Overview `Health Check` page evaluates a declarative six-rule bilingual catalog against the tenant-keyed Dashboard cache plus one isolated read-only enrichment query. Current Microsoft guidance means every AA/CQ resource account requires `PHONESYSTEM_VIRTUALUSER`; available service numbers use `PstnAssignmentStatus Unassigned` with `VoiceApplicationAssignment`. Rule toggles and finding suppressions persist per tenant in atomic local JSON, active findings flow into the same-tenant Tenant Report appendix, and supported findings link to their related in-app page. Existing ScriptBuilders, PowerShell command-facade output, and auth scopes remain byte-identical. English/German empty, populated, and suppressed states are in the headless matrix; native and tenant-connected execution remain separate pilot gates.
- 2026-09-10 — **Windows Teams WAM compatibility (3.29.1)**: MicrosoftTeams 7.9.0 changed user sign-in on Windows to WAM. Its broker integration derives the parent HWND from `GetConsoleWindow`, which is zero in PhoneDesk's hosted `WinExe` runspace; the legacy `MSAL_DISABLE_WAM` environment flags are not consumed by this module. `Connect-MicrosoftTeams` now uses Microsoft's documented temporary `-DisableWAM` compatibility switch. The bundled 7.9.0 parameter binding, 662 unit tests, 22 real-runspace integration tests, and the interactive Windows sign-in path without the prior handle error were verified; no tenant configuration was changed. Monitor Teams PowerShell release notes because Microsoft says the switch will be removed after supported alternatives are available.
- 2026-09-15 — Native Codex Code Review is enabled for this repository through the ChatGPT GitHub connector. Personal reviews run on PR open; CI keeps only the deterministic Semgrep scan, and no AI API key or Claude workflow is used.
