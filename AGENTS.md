...
- 2026-08-21 — **Task 1 localization core**: added typed `AppLanguage`/`UiTextKey`, `TranslationCatalog`, `ITranslationService`/`TranslationService`, and `IUserPreferencesStore`/`UserPreferencesStore` without touching XAML, bootstrap, `ViewModelBase`, or frozen infrastructure. Task 1 tests prove English default/fallback, German switch notifications plus persistence, named placeholder replacement, clear missing-key exceptions, and atomic `PhoneDesk/settings.json` writes using the lowercase `language` property. When running focused test filters on Windows, do not launch multiple `dotnet test` commands for the same project in parallel or MSBuild/Avalonia may lock `obj` outputs; sequential runs worked cleanly.
