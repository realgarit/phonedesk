...
- 2026-09-07 — **Issue #71 full live snapshots (3.28.0)**: topology schema v2 combines a newly queried Dashboard topology with structured data for every Tenant Report source (tenant, detailed AA/CQ flows, resource-account associations, schedules, phone-number assignments, and voice users). Comparisons validate the file before tenant access and fail closed on query errors, warnings, missing markers, or partial Dashboard sections; schema-v1 snapshots remain readable for their original fields. Nested drift is reported as scalar property changes. Detail rows that the frozen marker format identifies only by display name remain flat with deterministic occurrence indexes, so valid duplicate-name tenants do not fail or receive invented identity links. Existing Documentation/Dashboard ScriptBuilder output and authentication remain byte-identical; all snapshot queries are read-only. The full English/German headless matrix was visually inspected. Windows automation exposed only the older installed build, not the local 3.28.0 build, so native 3.28.0 and tenant-connected validation remain separate pilot gates.
