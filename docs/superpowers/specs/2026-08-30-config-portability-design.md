...
The topology document has its own fixed kind and schema version. Schema v2 captures the live Dashboard topology plus a structured superset of every Tenant Report dataset: tenant metadata, detailed auto-attendant call flows/menu options/schedule associations/operators, detailed call queues/agents/distribution lists/overflow/timeout actions, resource-account associations, schedules and ranges, phone-number inventory/assignments, and voice-enabled users/policies. Detail rows whose frozen source markers expose only a parent display name stay flat. A unique name resolves to the real parent identity and supports scalar property diffs; duplicate names remain explicitly unresolved and use deterministic full-row multiset identities, so a change is shown as added/removed rather than misattributed. Resource-account associations use a full `(account, type, configuration, occurrence)` identity so multiple associations of the same type cannot collide. Objects and nested ID lists are stable-sorted. `retrievedAtUtc` records collection time and is metadata, not a comparison field. Schema-v1 snapshots remain readable and compare their original topology fields.
